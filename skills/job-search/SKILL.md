...
**主策略：先定岗位大类，再筛方向**（避免漏岗的关键）

1. **主搜词用「岗位大类 + 城市」**，先把整个岗位池捞出来。例：方向是「金融科技产品经理」→ 主搜词是「产品经理」+ 杭州。
2. **行业/方向词不拼进主搜词**，而是对结果做**二次命中**：在岗位标题、描述、公司行业里找方向词（金融 / 金融科技 / 支付 / 信贷 / 风控 / 财富管理 / 数字银行 / 证券 / 基金 / AI / 大模型…），命中即标「🎯 方向命中」进候选。
3. 直接搜「金融产品经理」这类**复合主搜词是反例**：平台（尤其 Boss 直聘）按复合词做精确匹配，会漏掉标题写「高级产品经理（信贷方向）」的岗位，还常混入「金融销售 / 客户经理」这类非产品岗。复合词只作为补充加量，不主力。
...
### 用户想持续追踪
告诉用户：目前不支持自动定时搜索，但可以随时再跑一次 `/job-search`，AI 会用新的搜索结果。
